...
create_spec_df <- function(msfile) {
ms1scans <- header(msfile)[header(msfile)$retentionTime <= 1920 & header(msfile)$retentionTime >= 60 & header(msfile)$msLevel == 1,]
head(ms1scans)
...
if(ncol(scandf) > 0) {
if(min(abs(current_scan[i,1] - as.numeric(names(scandf)))) <= 0.025) {
...
scandf <- create_spec_df(msfile = msfile)
...
findpeak <- function(scandf, mz, rt_minutes, minscans = 5) {
...
for(x in 2:length(zero_scans)) {
  if(zero_scans[x]-zero_scans[x-1] - 1 > minscans) {
    ...
    mass_peaks[current_row,"firstScan"] <- row.names(curr_peak)[zero_scans[x-1]]
    mass_peaks[current_row,"lastScan"] <- row.names(curr_peak)[zero_scans[x]]
    mass_peaks[current_row,"peakScan"] <- row.names(curr_peak)[zero_scans[x-1] + which.max(curr_peak[zero_scans[x-1]:zero_scans[x],])-1]
  }}
